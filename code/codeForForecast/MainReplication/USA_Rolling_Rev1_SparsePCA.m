...
%% Initialization
jt              = 0;
nfactors        = zeros(leng_pred,length(vSparseLoad));
true            = zeros(leng_pred,HH(end),NN);
predAR          = zeros(leng_pred,HH(end),NN);
predSWgt        = zeros(leng_pred,HH(end),NN,rmax);
predSparsegt    = zeros(leng_pred,HH(end),NN,length(vSparseLoad),rmax);
opt_nlag_AR     = zeros(leng_pred,HH(end),NN);
opt_nlag_DIAR   = zeros(leng_pred,HH(end),NN,rmax);
opt_nlag_sparse = zeros(leng_pred,HH(end),NN,length(vSparseLoad),rmax);
bic_sparse      = zeros(leng_pred,HH(end),NN,length(vSparseLoad),rmax);
...
for j  = start_sample:end_sample-HH(end)
    ...
    Fsparse          = cell(length(vSparseLoad),1);

    parfor vlam = 1:length(vSparseLoad)
        %[Fmddm,lamhat,~,~,~] = factorMDDM(x, vSparseLoad(vlam), rmax);
        B = spca(x, [], rmax, inf, -vSparseLoad(vlam), 3000, 1e-3)
        Fsparse{vlam}  = x*B;
    end
    ...
    parfor k=1:NN
        ...
        predSparsegt2    = zeros(1,HH(end),1,length(vSparseLoad),rmax);
        opt_nlag_AR2     = zeros(1,HH(end),1);
        opt_nlag_DIAR2   = zeros(1,HH(end),1,rmax);
        opt_nlag_sparse2 = zeros(1,HH(end),1,length(vSparseLoad),rmax);
        bic_sparse2      = zeros(1,HH(end),1,length(vSparseLoad),rmax);
        for h=HH
            %% Model Selection
            ...
            for ff = 1:rmax
                ...
                for vlam = 1:length(vSparseLoad)
                    %% Sparse - Diffusion Index Forecast
                    Fsparse2                         = Fsparse{vlam}(:,1:ff);  %#ok<PFBNS>
                    [sLagSparse,sBicMD]              = fDiffusionIndexModSel(Fsparse2,y2f(:,k,h),xns(:,k),vV2,6,h);
                    opt_nlag_sparse2(1,h,1,vlam,ff)  = sLagSparse;
                    bic_sparse2(1,h,1,vlam,ff)       = sBicMD;
                    tempMDDMgt                       = fDiffusionIndexForecast(y2f(:,k,h),xns(:,k),Fsparse2,vV2,6,0,h);
                    predSparsegt2(1,h,1,vlam,ff)     = tempMDDMgt;
                end
            end
        end
        true(jt,:,k)                  = true2;
        ...
        predSparsegt(jt,:,k,:,:)      = predSparsegt2;
        opt_nlag_AR(jt,:,k)           = opt_nlag_AR2;
        opt_nlag_DIAR(jt,:,k,:)       = opt_nlag_DIAR2;
        opt_nlag_sparse(jt,:,k,:,:)   = opt_nlag_sparse2;
        bic_sparse(jt,:,k,:,:)        = bic_sparse2;
    end
    toc
end
clear x
