...
%%
% =================================================================
% 3. GENERATE THE COMPLETE LATEX TABLE FOR THE PAPER (BOTH PARTS)
% =================================================================
% This section runs ONCE after all simulations are complete.

fprintf('Simulations finished. Generating complete LaTeX code for Table 1...\n');

% --- Define Parameters and Open File ---
filename_latex = 'table1_complete_latex.tex';
fid = fopen(filename_latex, 'w');

r_values = 2:5;
dgp_values = 1:3;

% --- Write the Main LaTeX Header ---
fprintf(fid, '%% This table was automatically generated by MATLAB on %s\n\n', datetime('now'));
...
% --- Generate the Upper Part of the Table (n=50, T=50) ---
fprintf(fid, '%% Upper Part: n=50, T=50\n');
results_seq_upper = finalTable.sequentialTest_n50_T50;
results_eigen_upper = finalTable.eigenvalueRatio_n50_T50;
write_table_part(fid, r_values, dgp_values, results_seq_upper, results_eigen_upper);

% --- Write the Separator Between the Two Parts ---
fprintf(fid, '\\cmidrule{3-15}\n');
fprintf(fid, '& & & & & & & & & & & & & & \\\\\n'); % Blank spacer line

% --- Generate the Lower Part of the Table (n=50, T=100) ---
fprintf(fid, '%% Lower Part: n=50, T=100\n');
results_seq_lower = finalTable.sequentialTest_n50_T100;
results_eigen_lower = finalTable.eigenvalueRatio_n50_T100;
write_table_part(fid, r_values, dgp_values, results_seq_lower, results_eigen_lower);
...
fprintf('Success! Complete LaTeX table saved to %s\n', filename_latex);


% --- Helper function to write one part of the table ---
function write_table_part(fid, r_values, dgp_values, results_seq, results_eigen)
    index = 1; % Reset index for each part of the table
    for r_idx = 1:length(r_values)
        r = r_values(r_idx);
        for dgp_idx = 1:length(dgp_values)
            dgp = dgp_values(dgp_idx);
            
            % Get the simulation results for the current row
            ...
            % Handle the \multirow for the 'r' column
            if dgp_idx == 1
                % For the first DGP of a given r, print the r value with \multirow
                fprintf(fid, '\\multicolumn{2}{c|}{\\multirow{3}{*}{%d}} & ', r);
            else
                % For subsequent DGPs, leave the first column blank but keep the structure
                fprintf(fid, '\\multicolumn{2}{c|}{} & ');
            end
            
            % Print the DGP label and the formatted data for the rest of the row
        ...
        % Add a blank spacer row between r-groups, but not after the very last one
