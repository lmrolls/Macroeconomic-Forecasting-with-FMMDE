...
filename = 'Simulation_ResultsTAB1.xlsx';
sheet = 'Table1';
...
disp('Results saved to Simulation_ResultsTAB1.xlsx');
...
fprintf('Simulations finished. Generating final, aligned LaTeX table...\n');

% --- Define Parameters and Open File ---
filename_latex = 'table1_final_version.tex';
...
fprintf(fid, '  \\caption{Empirical probability of estimating the true number of factors for n=50, T=50 (upper) and T=100 (lower).}\n');
fprintf(fid, '  \\label{tab:full_simulation_table}\n');
fprintf(fid, '  \\scalebox{0.7}{\n'); % Start the scalebox
fprintf(fid, '    %% -- Start of table content --\n');
fprintf(fid, '    \\begin{tabular}{cc|ccccccc|cccccc|}\n');
fprintf(fid, '    \\cmidrule{3-15}\n');

% MODIFIED: This header line now has one less '&' in the middle for correct alignment.
...
% MODIFIED: This header line also has one less '&' in the middle.
...
fprintf(fid, '    %% -- End of table content --\n');
fprintf(fid, '  } %% End of scalebox\n');
fprintf(fid, '\\end{table}\n');
fclose(fid);

fprintf('Success! Final LaTeX table saved to %s\n', filename_latex);


% --- Helper function (this remains unchanged from the last version) ---
