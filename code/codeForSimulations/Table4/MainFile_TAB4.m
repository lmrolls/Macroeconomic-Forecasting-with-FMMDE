...
%                              %
%==============================%
filename = 'Simulation_ResultsTAB4.xlsx';
sheet = 'Results';
...
fprintf('Simulations for Table 4 finished. Generating LaTeX code...\n');

% --- Define Parameters and Open File ---
filename_latex = 'table4_final_version.tex';
fid = fopen(filename_latex, 'w');

% Get parameter values from your script's variables
k0_values = k0_start;
T_values = TT_start;
N_values = NN_start;
...
fprintf(fid, '  \\caption{Empirical probability of estimating the true number of factors for the nonlinear DGP.}\n');
fprintf(fid, '  \\label{tab:nonlinear_dgp_sim_table}\n');
fprintf(fid, '  \\scalebox{0.8}{\n');
% Define the table structure with vertical lines
fprintf(fid, '    \\begin{tabular}{cc|ccc|ccc}\n');
fprintf(fid, '    \\toprule\n');

% Write the main headers spanning multiple columns
fprintf(fid, '    \\multicolumn{2}{c|}{} & \\multicolumn{3}{c|}{\\textbf{Sequential Test}} & \\multicolumn{3}{c}{\\textbf{Eigenvalue Ratio}} \\\\\n');
fprintf(fid, '    \\cmidrule(lr){3-5} \\cmidrule(lr){6-8}\n');

% Write the sub-headers
fprintf(fid, '    $k_0$ & \\diagbox{$T$}{$n$} & %d & %d & %d & %d & %d & %d \\\\\n', N_values(1), N_values(2), N_values(3), N_values(1), N_values(2), N_values(3));
fprintf(fid, '    \\midrule\n');

% --- Loop Through Results and Write Table Body ---
for k0_idx = 1:length(k0_values)
    k0 = k0_values(k0_idx);
    for T_idx = 1:length(T_values)
        T = T_values(T_idx);

        % For the first row of a k0-block, write the k0 value in a multirow cell
        if T_idx == 1
            fprintf(fid, '    \\multirow{3}{*}{%d} & ', k0);
        else
            % For subsequent rows, leave this cell blank
            fprintf(fid, '    & ');
        end

        % Write the T value for the current row
        fprintf(fid, '%d & ', T);
        
        % Extract the data for the entire row from your results matrices
        seq_data = [mOutTest(T_idx, 1, k0_idx), mOutTest(T_idx, 2, k0_idx), mOutTest(T_idx, 3, k0_idx)];
        eigen_data = [mOutRAT(T_idx, 1, k0_idx), mOutRAT(T_idx, 2, k0_idx), mOutRAT(T_idx, 3, k0_idx)];
        
        % Print the formatted data for both parts of the table
        fprintf(fid, '%.2f & %.2f & %.2f & ', seq_data);
        fprintf(fid, '%.2f & %.2f & %.2f \\\\\n', eigen_data);
    end
    
    % Add a rule between the k0 blocks for clarity
    if k0_idx < length(k0_values)
       fprintf(fid, '    \\midrule\n');
    end
end
...
fprintf('Success! Final LaTeX table for Table 4 saved to %s\n', filename_latex);