...
predLAMfix      = zeros(leng_pred,HH(end),NN,length(K0max),rmax);
...
for j  = start_sample:end_sample-HH(end)
    ...
    parfor k0 = 1:length(K0max)
        Fmddmtemp            = fixedFactorLAM(x, K0max(k0),rmax);
        Fmddm                = Fmddmtemp(:,1:rmax);
        FmddmK0{k0}          = Fmddm;
    end
...
file_name        = 'USA_Rolling_Rev1_FixedFactors_LAM_v2.mat';
save(fullfile(output_folder, file_name), 'predLAMfix');
