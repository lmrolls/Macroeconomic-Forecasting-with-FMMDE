%% To uncomment if you are using a Super Computer
clear; clc;
% %%%%%%%%%%%%%%%%%%%%%%%%% LOADING THE DATA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% The number of lags for Covid Indicators
cp = 3;
...
cBartWin = round(sqrt(start_sample));
for j  = start_sample:end_sample-HH(end)
    ...
    for k0 = 1:length(K0max)
        [Fmddmtemp,lamhat,~,~,icstar]   = factorLAM(x, K0max(k0));
        Fmddm                           = Fmddmtemp(:,1:icstar);
        [~,nfacLAM(jt,k0)]             = size(Fmddm);
        FmddmK0{k0}                     = Fmddm;
        lamhat                          = lamhat(:,1:nfacLAM(jt,k0));
    end
