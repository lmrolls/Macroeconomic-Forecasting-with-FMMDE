...
predMdscree     = zeros(leng_pred,HH(end),NN,length(K0max));
...
for j  = start_sample:end_sample-HH(end)
    ...
    parfor k0 = 1:length(K0max)
        [Fmddmtemp,lamhat,~,~,icstar]   = factorMDDM(x, k0, rmax);
        Fmddm                           = Fmddmtemp(:,1:icstar);
        FmddmK0{k0}                     = Fmddm;
        nScreeFac(jt,k0)                = size(Fmddm,2);
    end
...
save(fullfile(output_folder, file_name), 'predMdscree');