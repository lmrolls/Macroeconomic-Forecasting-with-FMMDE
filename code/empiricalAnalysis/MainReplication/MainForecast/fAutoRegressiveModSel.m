function [bestModBIC,sBic,bic] = fAutoRegressiveModSel(yf,y,vV,pmax,h)
% This function selects the optimal number of lags for an autoregressive model
% based on the Bayesian Information Criterion (BIC).
% Inputs:
%   yf    - Forecast target variable (column vector)
%   y     - Original dependent variable used for lag construction
%   vV    - Additional regressors (can be empty)
...
%   bic        - Vector of BIC values for each lag order

% Adjust yf and lagged y matrix to match after maximum lag
yf = yf(pmax + 1 : end, :);
...
for nv = 1:pmax+1
    ...
    % Build the regressor matrix X
    if ind == 1
        X = ones(length(yf),1); % Only intercept if no lags
    else
        X = [ones(length(yf),1) Ly(:,1:ind-1)]; % Intercept and lagged values
    end
    ...
    % Estimate model coefficients by OLS including additional regressors
