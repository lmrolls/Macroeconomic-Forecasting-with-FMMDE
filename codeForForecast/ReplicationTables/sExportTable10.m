...
for k = 1:NN
    for h = HH
        ...
        MSFE_STRUCT(k,h).MDDM_ST        = mean((finaltrue(:,k,h) - finalpredMD(:,k,h)).^2);
        MSFE_STRUCT(k,h).MDDM_SCREE     = mean((finaltrue(:,k,h) - finalpredScreeMD(:,k,h)).^2);
        MSFE_STRUCT(k,h).LAM            = mean((finaltrue(:,k,h) - finalpredLAM(:,k,h)).^2);
        MSFE_STRUCT(k,h).SW             = mean((finaltrue(:,k,h) - finalpredSW(:,k,h)).^2);
        MSFE_STRUCT(k,h).MDDM_CV        = mean((finaltrue(:,k,h) - finalpredFixMD(:,k,h)).^2);
    end
end
