...
for k = 1:NN
    for h = HH
        ...
        MSFE_STRUCT(k,h).ICA            = mean((finaltrue(1:en-h,k,h) - finalpredICA(1:en-h,k,h)).^2);
        MSFE_STRUCT(k,h).SPARSE         = mean((finaltrue(1:en-h,k,h) - finalpredSparse(1:en-h,k,h)).^2);
        MSFE_STRUCT(k,h).BOOSTING       = mean((finaltrue(1:en-h,k,h) - finalpredBoosting(1:en-h,k,h)).^2);
        MSFE_STRUCT(k,h).COMBO_EQ       = mean((finaltrue(1:en-h,k,h) - ComboEqual(1:en-h,k,h)).^2);
        MSFE_STRUCT(k,h).COMBO_IP       = mean((finaltrue(1:en-h,k,h) - ComboInvPer(1:en-h,k,h)).^2);
    end
end
