...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%           LATEX TABLE GENERATION FOR TABLE 5
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

fprintf('Simulations for Table 5 finished. Generating LaTeX code...\n');

% --- Define Parameters and Open File ---
filename_latex = 'table5_final_version.tex';
...
fprintf(fid, '    \\caption{Ratio of the average one step ahead mean square forecast errors (MSFE) computed for the two models (see equations \\ref{eq:nonlinearEQ} and \\ref{eq:linearM}) over 1000 simulations, FMMDE (Numerator), SW and LYB model (Denominator)}\n');
fprintf(fid, '    \\label{tab:Tab1SimulLinear}\n');
fprintf(fid, '    \\scalebox{0.7}{\t\\begin{tabular}{r|rrr|rrr|lll|lll|}\n');
fprintf(fid, '        \\toprule\\toprule\n');

% Main Headers
fprintf(fid, '        \\multicolumn{1}{r}{} & \\multicolumn{6}{c}{SW Model} & \\multicolumn{6}{c}{LYB Model} \\\\\n');
fprintf(fid, '        \\cmidrule{2-13}\n');

% Sub Headers
fprintf(fid, '        & \\multicolumn{3}{c}{Linear Factors} & \\multicolumn{3}{c|}{Nonlinear Factors} & \\multicolumn{3}{c}{Linear Factors} & \\multicolumn{3}{c|}{Nonlinear Factors} \\\\\n');
fprintf(fid, '        \\cmidrule{1-13}\n');

% T-value Headers
fprintf(fid, '        \\diagbox{$n$}{$T$} & 200 & 500 & \\multicolumn{1}{c|}{1000} & 200 & 500 & \\multicolumn{1}{c|}{1000} & \\multicolumn{1}{c}{200} & \\multicolumn{1}{c}{500} & \\multicolumn{1}{c|}{1000} & \\multicolumn{1}{c}{200} & \\multicolumn{1}{c}{500} & \\multicolumn{1}{c|}{1000} \\\\\n');
fprintf(fid, '        \\cmidrule{1-13}\n');

% --- Loop Through Results and Write Table Body ---
for NN = 1:length(NN_start)
    % Print the N value (row header)
    fprintf(fid, '        %d & ', NN_start(NN));
    
    % Print data for the row, formatted to 3 decimal places
    fprintf(fid, '%.3f & %.3f & %.3f & ', rMSFE_SW_lin(NN, :));
    fprintf(fid, '%.3f & %.3f & %.3f & ', rMSFE_SW_nonlin(NN, :));
    fprintf(fid, '%.3f & %.3f & %.3f & ', rMSFE_LYB_lin(NN, :));
    fprintf(fid, '%.3f & %.3f & %.3f \\\\\n', rMSFE_LYB_nonlin(NN, :));
    
    % Add a small space between rows if desired
    if NN < length(NN_start)
        fprintf(fid, '        \n');
    end
end
...
fprintf('Success! Final LaTeX table for Table 5 saved to %s\n', filename_latex);
